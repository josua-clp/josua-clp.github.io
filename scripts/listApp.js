document.addEventListener('DOMContentLoaded', () => {
    ...
    function addTask() {
        ...
        const taskElement = createTaskElement(taskText, taskDesc, taskDate);
        document.querySelector('.task-container').appendChild(taskElement); 
        saveTasksToLocalStorage();
    }
    
    function createTaskElement(text, desc, date) {
    ...
    function saveTasksToLocalStorage() {
        ...
        containers.forEach(container => {
            container.querySelectorAll('.task').forEach(task => {
                ...
                const title = task.querySelector('h1') ? task.querySelector('h1').textContent : ''; // Ensure title is correctly fetched
                const desc = task.querySelector('p') ? task.querySelector('p').textContent : ''; // Ensure description is correctly fetched
                const dateSpan = task.querySelector('span'); // Fetch the first span, assuming it's for the date
                const date = dateSpan ? dateSpan.textContent.replace('🕗 ', '') : ''; // Ensure date is correctly fetched
    ...
    function loadTasksFromLocalStorage() {
        ...
        tasks.forEach(task => {
            // Use a single container for all tasks
            const container = document.querySelector('.task-container');
            if (container) {
                const taskElement = createTaskElement(task.title, task.desc, task.date);
